...
const AxisCollection& ResponseMatrix::GetAxes() const{
    return fAxes;
}

void ResponseMatrix::SetResponse(const std::vector<std::vector<double> >& response_){
    fResponse = response_;
}

void ResponseMatrix::SetRow(size_t index_, const std::vector<double>& row_){
    fResponse[index_] = row_;
}

void ResponseMatrix::SetColumn(size_t index_, const std::vector<double>& col_){
    for(size_t i = 0; i < fNBins; i++){
        fResponse[i][index_] = col_[i];
    }
}

void ResponseMatrix::SetComponent(size_t col_, size_t row_, double val_){
    fResponse[col_][row_] = val_;
}

ResponseMatrix ResponseMatrix::operator = (const ResponseMatrix& other_){
    if(this != &other_){
        fAxes  = other_.fAxes;
...
BinnedPdf ResponseMatrix::operator()
