...
void
IO::SaveDataSet(const DataSet& dataSet_, const std::string& filename_, const std::string& treename){
    std::string ext = GetExt(filename_);
    if(ext == "h5")
        SaveDataSetH5(dataSet_, filename_);

    else if(ext == "root")
        SaveDataSetROOT(dataSet_, filename_, treename);
...
void
IO::SaveDataSetROOT(const DataSet& dataSet_, const std::string& filename_, const std::string& treename){
    ...
    TNtuple nt(treename.c_str(), "", branchString.c_str());
