...
Histogram::GetAllProjections() const{
  ...
  std::vector<std::vector<size_t> > projectionIndices = AllCombinationsShorterThan<size_t>(SequentialElements(size_t(0), fNDims), 2);

  for(size_t i = 0; i < projectionIndices.size(); i++){
	std::vector<size_t> indicesToKeep = projectionIndices.at(i);
	// FIX ME - a bit hacky
	if(std::set<size_t>(indicesToKeep.begin(), indicesToKeep.end()).size()
	   != indicesToKeep.size())
	  continue;
